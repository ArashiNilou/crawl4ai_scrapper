...
This project is a web crawler built with Python that extracts exposant data (wedding reception exposants) from a website using asynchronous programming with Crawl4AI. It utilizes a language model-based extraction strategy and saves the collected data to a CSV file.

## Features

- Asynchronous web crawling using [Crawl4AI](https://pypi.org/project/Crawl4AI/)
- Data extraction powered by a language model (LLM)
- CSV export of extracted exposant information
...
├── models
│ └── exposant.py # Defines the exposant data model using Pydantic
├── utils
...
The script will crawl the specified website, extract data page by page, and save the complete exposants to a `complete_exposants.csv` file in the project directory. Additionally, usage statistics for the LLM strategy will be displayed after crawling.

## Configuration

The `config.py` file contains key constants used throughout the project:

- **BASE_URL**: The URL of the website from which to extract exposant data.
- **CSS_SELECTOR**: CSS selector string used to target exposant content.
- **REQUIRED_KEYS**: List of required fields to consider a exposant complete.
